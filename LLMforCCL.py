...
llm = ChatGoogleGenerativeAI(
    model="gemini-1.5-flash", # Type of model chosen
...
def data_indexing(folder_path):
    ...
    index.storage_context.persist(persist_dir="./ccl_gpt_store")

    return index

# This function returns a response by querying the data store.
def data_querying(input_text):
    storage_context = StorageContext.from_defaults(persist_dir="./ccl_gpt_store")
...
folder_path = 'TAMS CCL User Guide'
...
iface = gr.Interface(fn=data_querying,
                     ...
                     outputs="text",
                     title="TAMS CCL User Guide Chatbot")
